...
function DialogTrigger({ children, ...props }: any) {
  ...
  return (
    <div onClick={() => setOpen(true)} className="inline-block" {...props}>
      {children}
    </div>
  )
}
  ...
  return (
    <DialogPortal>
      <div className="fixed inset-0 z-[1000] flex items-center justify-center p-4">
        ...
        <div
          className={cn(
            "relative z-[1001] w-full max-w-lg rounded-xl bg-white p-6 text-slate-900 shadow-2xl ring-1 ring-slate-200 outline-none",
            className
          )}
          {...props}
        >
...
function DialogClose({ children, ...props }: any) {
  const { setOpen } = React.useContext(DialogContext)
  return (
    <div onClick={() => setOpen(false)} className="inline-block" {...props}>
      {children}
    </div>
  )
}
